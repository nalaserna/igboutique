...
function moon_shop_Widget_layed_nav() {
    register_widget( 'Moon_Widget_Layered_Nav' );
}

add_action( 'widgets_init' , 'moon_shop_Widget_layed_nav' );

/**
 * Widget layered nav class.
 */
class Moon_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'moon-shop' );
		$this->widget_id          = 'moon_shop_layered_nav';
		$this->widget_name        = __( 'Moon Filter Products by Attribute', 'moon-shop' );
        parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'        => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'moon-shop' ),
				'label' => __( 'Title', 'moon-shop' ),
			),
			'attribute'    => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'moon-shop' ),
				'options' => $attribute_array,
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'moon-shop' ),
				'options' => array(
					'list'     => __( 'List', 'moon-shop' ),
					'dropdown' => __( 'Dropdown', 'moon-shop' ),
				),
			),
			'query_type'   => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'moon-shop' ),
				'options' => array(
					'and' => __( 'AND', 'moon-shop' ),
					'or'  => __( 'OR', 'moon-shop' ),
				),
			),
		);
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'moon-shop' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
			...
			if ( $multiple ) {
				echo '<button class="woocommerce-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'moon-shop' ) . '">' . esc_html__( 'Apply', 'moon-shop' ) . '</button>';
			}
				...
				if ( jQuery().selectWoo ) {
					var wc_layered_nav_select = function() {
						jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).selectWoo( {
							...
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'moon-shop' ) ) . "';
								}
							}
						} );
					};
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		// List display.
		echo '<ul class="woocommerce-widget-layered-nav-list moon-layed-nav">';
		...
		foreach ( $terms as $term ) {
			$widget_swatch = get_option($taxonomy.'_'.$term->term_id);
			$current_values = isset( $_chosen_attributes[ $taxonomy ]['terms'] ) ? $_chosen_attributes[ $taxonomy ]['terms'] : array();
			$option_is_set  = in_array( $term->slug, $current_values, true );
			$count          = isset( $term_counts[ $term->term_id ] ) ? $term_counts[ $term->term_id ] : 0;
			...
			if (!empty($widget_swatch) && $widget_swatch['enable-swatch'] == 'on') {
				$style = '';

				if( ! empty( $widget_swatch['swatch-image-url'] ) ) {
					$style = 'background-image: url(' . esc_url($widget_swatch['swatch-image-url']) . '); background-size: cover; background-repeat: no-repeat;';
				} else if( ! empty( $widget_swatch['swatch-color'] )) {
					$style = 'background-color:' .  $widget_swatch['swatch-color'];
				}
				if (!empty($style)) {
					if ( $count > 0 || $option_is_set ) {
						$link      = esc_url( apply_filters( 'woocommerce_layered_nav_link', $link, $term, $taxonomy ) );
						$term_html = '<a rel="nofollow" class="moon-widget-swatch" href="' . $link . '" title="' . esc_html( $term->name ) . '" style="'.$style.'"></a>';
					} else {
						$link      = false;
						$term_html = '<a rel="nofollow" class="moon-widget-swatch" href="' . $link . '" title="' . esc_html( $term->name ) . '" style="'.$style.'"></a>';
					}
